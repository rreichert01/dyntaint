...
;(traces -->T (load-T (term (
;                            (alloc-source "Src_42_3" 42 3)
;                            (alloc-prop "Prop_3->2" tainted_var 3 2)
;                            (alloc-sanit "Sanit_2->0" tainted_var 2 0)
;                            (alloc-sink "Sink_0" y 0 y)
;                            (= tainted_var alloc)
;                            (= untainted_var alloc)
;                            (apply-source "Src_42_3" tainted_var)
;                            (apply-prop "Prop_3->2" tainted_var tainted_var)
;                            (apply-sanit "Sanit_2->0" tainted_var untainted_var)
;                            (apply-sink "Sink_0" untainted_var)))))

;(traces -->T (load-T (term (
;                            (alloc-source "Src_42_3" 42 3)
;                            (alloc-prop "Prop_3->2" tainted_var 3 2)
;                            (alloc-sanit "Sanit_2->1" tainted_var 2 1)
;                            (alloc-sink "Sink_0" y 0 y)
;                            (= tainted_var alloc)
;                            (= untainted_var alloc)
;                            (apply-source "Src_42_3" tainted_var)
;                            (apply-prop "Prop_3->2" tainted_var tainted_var)
;                            (apply-sanit "Sanit_2->1" tainted_var untainted_var)
;                            (apply-sink "Sink_0" untainted_var)))))


;(traces -->T (load-T (term (
;                            (alloc-source "Src_42_3" 42 3)
;                            (alloc-prop "Prop_3->2" (mult (add tainted_var 32) 137) 3 2)
;                            (alloc-sanit "Sanit_2->0" tainted_var 2 0)
;                            (alloc-sink "Sink_0" m 0 h)
;                            (= tainted_var alloc)
;                            (= untainted_var alloc)
;                            (= y alloc)
;                            (apply-source "Src_42_3" tainted_var)
;                            (= y (add 2 (mult 2 (div tainted_var 3))))
;                            (apply-prop "Prop_3->2" tainted_var tainted_var)
;                            (apply-sanit "Sanit_2->0" tainted_var untainted_var)
;                            (apply-sink "Sink_0" untainted_var)
;                            (apply-sink "Sink_0" y)))))
...
;TRACES
;(traces -->T (load-T (term ((= x alloc) (if true (= x 1) (= x 0))))))
;(traces -->T (load-T (term ((= x alloc) (if false (= x 1) (= x 0))))))

;(traces -->T (load-T (term ((= x alloc) (while true ((= x 4) (= x 2))) (= x 1)))))
;(traces -->T (load-T (term ((= x alloc) (while false ((= x 4) (= x 2))) (= x 1)))))

;(traces -->T (load-T (term (alloc-source "source1" (add 1 1) 0))))
;(traces -->T (load-T (term ((= x alloc) (apply-source "source1" x)))))

;(traces -->T (load-T (term (alloc-sink "sink1" 1 0 1))))
;(traces -->T (load-T (term (if true (alloc-sink "sink1" 1 0 1) (alloc-source "source1" 1 0)))))
